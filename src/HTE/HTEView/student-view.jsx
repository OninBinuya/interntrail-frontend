...
import { Grid, List, Eye, Filter, TrophyIcon, MoreHorizontal, Users, Search, CheckCircle2, XCircle, FileText, Award, Download, UserCheck, Clock, ChevronDown, Building2 } from "lucide-react"
              ...
              <Table>
                ...
                <TableBody>
                  {progressReports
                    ...
                    .map((student) => {
                      // Calculate actual completed and remaining hours
                      const totalHours = parseInt(student.remaining_hours || 360);
                      const completedHours = student.total_hours_rendered
                        ? parseFloat(student.total_hours_rendered).toFixed(2)
                        : parseInt(student.hours_completed || 0);

                      console.log({ student, completedHours, totalHours });

                      const actualRemainingHours = Math.max(0, totalHours - completedHours).toFixed(2);
                      const progressPercentage = Math.min(100, Math.round((completedHours / totalHours) * 100));

                      return (
                        <TableRow key={student.id || student.traineeID}>
                          <TableCell className="font-medium">
                            <div className="flex items-center gap-3">
                              <div className="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center">
                                {student.avatar ? (
                          ...
                          <TableCell>
                            <div className="flex flex-col">
                              <span className="text-sm font-medium">{completedHours} / {totalHours} hours</span>
                              <span className="text-xs text-gray-500">{actualRemainingHours} hours remaining</span>
                            </div>
                          </TableCell>
                          <TableCell className="text-right">
                            <Popover open={openMenuId === student.traineeID} onOpenChange={(open) => setOpenMenuId(open ? student.traineeID : null)}>
